...
<div class="h-screen w-screen bg-background-primary" bind:this={container}>
    <div class="relative flex h-screen flex-row flex-nowrap overflow-hidden" bind:this={slider}>
        <div
            class="flex h-screen w-screen shrink-0 items-center justify-center bg-background-primary font-yeseva text-7xl capitalize"
        >
            <h2 bind:this={intro} class="text-6xl sm:text-7xl md:text-8xl lg:text-9xl">My Craft</h2>
        </div>
        {#each [1, 2, 3] as i (i)}
            <div
                class="flex h-screen w-screen shrink-0 items-center justify-center bg-background-primary text-3xl"
            >
                Slide {i}
            </div>
        {/each}
